...
export async function POST(request: Request) {
  try {
    ...
    const organization = await prisma.organization.findUnique({ where: { id: body.organizationId }, select: { id: true } });
    if (!organization) return NextResponse.json({ error: 'Association introuvable' }, { status: 404 });
