...
export function DonationBlock({ content, organizationId }: { content: any; organizationId?: string }) {
  ...
  const paymentUrl = content.stripeUrl || content.helloAssoUrl || '';
  ...
  async function submit(event: React.FormEvent) {
    ...
    const response = await fetch('/api/public/donations', {
      ...
      body: JSON.stringify({ ...form, organizationId, amountEuros: total, method: content.stripeUrl ? 'STRIPE' : 'HELLOASSO' }),
    });
  ...
  return <div className="donation-block mx-auto max-w-3xl rounded-3xl bg-white p-5 text-left shadow-lg ring-1 ring-gray-200 md:p-9">
    ...
    {content.cardEnabled && paymentUrl && <form onSubmit={submit} className="mt-8 space-y-5">
      ...
      <button disabled={state === 'sending' || total <= 0} className="btn btn-primary w-full py-4 text-base disabled:cursor-not-allowed disabled:opacity-50"><CreditCard className="h-5 w-5" />{state === 'sending' ? (en ? 'Preparing secure payment…' : 'Préparation du paiement sécurisé…') : total > 0 ? (en ? `Continue to secure payment · €${total}` : `Continuer vers le paiement sécurisé · ${total} €`) : (en ? 'Choose an amount to continue' : 'Choisir un montant pour continuer')}</button>
      <div className="flex flex-wrap justify-between gap-2 border-t pt-4 text-xs text-gray-500"><span className="flex items-center gap-1"><CheckCircle2 className="h-4 w-4 text-green-600" /> {en ? 'Donation recorded in donor CRM' : 'Don enregistré dans le CRM donateurs'}</span><span className="flex items-center gap-1"><LockKeyhole className="h-4 w-4" /> {en ? 'Secure payment' : 'Paiement sécurisé'}</span></div>
    </form>}
    ...
    {!content.cardEnabled && !content.transferEnabled && !content.chequeEnabled && <p className="mt-6 text-center text-gray-500">{en ? 'Donation methods will be available soon.' : 'Les moyens de don seront bientôt disponibles.'}</p>}
  </div>;
}
