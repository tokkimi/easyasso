...
const socialKey = (label: string) =>
  label.toLowerCase().replace(/[^a-z0-9]/g, "");
const isSocialColumn = (column: FooterConfig["columns"][number]) =>
  column.links.length > 0 &&
  column.links.every((link) => SOCIAL_LABELS.has(socialKey(link.label)));
const logoFrameClass =
  "inline-flex max-w-[220px] items-center rounded-xl bg-transparent p-0.5 sm:max-w-[280px]";
...
function SocialMark({
  ...
  const assets: Record<string, string> = {
    ...
    youtubemusic:
      "https://cdn.jsdelivr.net/npm/simple-icons@latest/icons/youtubemusic.svg",
    ...
    amazonmusic:
      "https://cdn.jsdelivr.net/npm/simple-icons@latest/icons/amazonmusic.svg",
    beatport:
      "https://cdn.jsdelivr.net/npm/simple-icons@latest/icons/beatport.svg",
    bandcamp:
      "https://cdn.jsdelivr.net/npm/simple-icons@latest/icons/bandcamp.svg",
  ...
  if (asset)
    return (
      <img
        src={asset}
        alt=""
        className={`h-[18px] w-[18px] object-contain ${monochrome ? "brightness-0 invert" : asset.startsWith("http") ? "rounded bg-white/90 p-0.5" : ""}`}
      />
    );
  if (key === "shotgun")
    return (
  ...
  if (key === "tiktok")
    return (
      ...
      >
        <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.9 2.9 0 1 1-2-2.76v-3.5a6.34 6.34 0 1 0 5.45 6.26V8.73a8.16 8.16 0 0 0 4.77 1.52V6.8c-.34 0-.67-.04-1-.11Z" />
      </svg>
    );
  ...
  if (key === "x" || key === "twitter")
    return <span className="text-base font-medium leading-none">𝕏</span>;
  return null;
}
...
export function PublicHeader({
  ...
  const socials = Object.entries(header.social || {}).filter(
    (entry): entry is [string, string] => Boolean(entry[1]),
  );
  const brandedHeader =
    header.logoUrl?.includes("/vielusos/") ||
    header.background?.toLowerCase() === "#0b0b10";
  const glassMenu = !brandedHeader && (header.menuGlass ?? true);
  const menuOpacity =
    Math.max(20, Math.min(100, Number(header.menuOpacity) || 78)) / 100;
  ...
  return (
    ...
    >
      <div className="relative flex w-full items-center justify-between gap-3 px-3 py-3 sm:px-5 lg:px-8">
        <Link
          href={basePath || "/"}
          className="flex min-w-0 flex-1 basis-0 items-center justify-start text-lg font-extrabold"
        >
          {header.logoUrl ? (
              ...
              <nav className="flex min-w-0 items-center justify-center gap-2 text-center text-sm font-medium lg:gap-4">
                {nav.map((p) => (
                  <Link
                    ...
                    className="max-w-[10rem] break-words leading-tight opacity-80 hover:opacity-100"
                  >
                    {t(p.title)}
                  </Link>
                ))}
        ...
        <div className="flex flex-1 basis-0 items-center justify-end gap-2.5">
          {!brandedHeader &&
            socials.map(([name, href]) => (
              <a
                ...
                aria-label={name}
                title={name}
                className="hidden h-7 w-7 shrink-0 items-center justify-center text-current opacity-80 transition hover:opacity-100 lg:inline-flex"
              >
                <SocialMark label={name} monochrome={false} />
              </a>
            ))}
          {!brandedHeader && (
          ...
          <button
            ...
            className={`public-header-menu-button touch-target shrink-0 items-center justify-center rounded-xl border ${brandedHeader ? "!flex border-white/20 bg-transparent text-white hover:bg-white/10" : "border-black/10 bg-white/80"}`}
        ...
        {menuOpen && (
          <div
            style={
              !brandedHeader
                ? {
                    background: glassMenu
                      ? hexToRgba(menuBackground, menuOpacity)
                      : menuBackground,
                    color: glassMenu ? "#fff" : header.textColor,
                    backdropFilter: glassMenu
                      ? `blur(${Math.max(0, Math.min(36, Number(header.menuBlur) || 20))}px)`
                      : undefined,
                  }
                : undefined
            }
          ...
          >
            ...
            <div
              className={`mt-3 flex flex-wrap items-center gap-3 px-2 pt-3 ${brandedHeader || glassMenu ? "border-t border-white/10" : "border-t border-black/10"}`}
            >
              {socials.map(([name, href]) => (
                <a
                  key={name}
                  ...
                  rel="noreferrer"
                  aria-label={name}
                  className={`grid h-10 w-10 place-items-center rounded-full border ${brandedHeader || glassMenu ? "border-white/20 text-white/80 hover:bg-white/10 hover:text-white" : "border-black/15 text-current"}`}
                >
                  <SocialMark
                    label={name}
                    monochrome={brandedHeader || glassMenu}
                  />
                </a>
              ))}
              <LanguageSwitcher variant="inline" />
            ...
            {brandedHeader && (
              <Link
                href={customerHref}
                onClick={() => setMenuOpen(false)}
                className="mt-3 flex min-h-11 items-center justify-center gap-2 rounded-xl border border-white/15 px-4 text-sm font-semibold text-white/85 transition hover:bg-white/10 hover:text-white"
              >
            ...
            {!brandedHeader && header.showCta !== false && cta?.text?.trim() && cta?.href?.trim() && (
              <a
                ...
                className="mt-2 flex min-h-11 items-center justify-center rounded-xl px-4 text-sm font-bold"
              >
                {cta.text}
              </a>
            )}
...
export function PublicFooter({
  ...
  const footerHref = (href: string, label: string) => {
    ...
    const page =
      nav.find((item) => item.slug === slug) ||
      nav.find((item) => normalize(item.title) === normalize(label));
    return page
      ? page.isHome
        ? basePath || "/"
        : `${basePath}/${page.slug}`
  ...
  const selectedPages = nav.filter(
    (page) => !footer.pageSlugs || footer.pageSlugs.includes(page.slug),
  );
  const socialColumns = (footer.columns || []).filter(isSocialColumn);
  const brandedFooter =
    footer.logoUrl?.includes("/vielusos/") ||
    footer.background?.toLowerCase() === "#0b0b10";
  return (
    <footer
      style={{
      ...
      className="public-footer-shell mt-0"
    >
      <div className="public-footer-grid mx-auto grid max-w-5xl gap-8 px-4 py-12">
            ...
            <ul className="mt-3 space-y-2 text-sm opacity-80">
              {selectedPages.map((page) => (
                <li key={page.slug}>
                  <a
                    href={
                      page.isHome ? basePath || "/" : `${basePath}/${page.slug}`
                    }
        ...
        {socialColumns.map((col, i) => (
          <div key={i}>
            ...
            <ul
              className={`mt-3 text-sm opacity-80 ${isSocialColumn(col) ? "flex flex-wrap gap-2" : "space-y-2"}`}
            >
              {col.links.map((l, j) => (
                <li key={j}>
                  <a
                    href={footerHref(l.href, l.label)}
                    target={isSocialColumn(col) ? "_blank" : undefined}
                    rel={isSocialColumn(col) ? "noreferrer" : undefined}
                    title={t(l.label)}
                    aria-label={t(l.label)}
                    className={
                      isSocialColumn(col)
                        ? "grid h-11 w-11 place-items-center rounded-xl border border-current/20 transition hover:bg-white/10 hover:opacity-100"
                        : "hover:opacity-100"
                    }
                  >
                    {isSocialColumn(col) ? (
                      <SocialMark label={l.label} monochrome={brandedFooter} />
                    ) : (
                      t(l.label)
                    )}
                  </a>
      ...
      <div className="border-t border-white/10">
        <div className="public-footer-bottom mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-2 px-4 py-4 text-xs opacity-70">
          ...
          <div className="flex gap-4">
            {footer.showCgv && (
              <a href={`${basePath}/cgv`} className="hover:opacity-100">
                CGV
              </a>
            )}
            {footer.showMentions && (
