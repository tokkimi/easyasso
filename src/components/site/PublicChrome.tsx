...
const SOCIAL_LABELS = new Set(['facebook', 'instagram', 'linkedin', 'youtube', 'youtubemusic', 'spotify', 'deezer', 'soundcloud', 'applemusic', 'amazonmusic', 'beatport', 'bandcamp', 'tidal', 'shotgun', 'tiktok', 'x', 'twitter', 'email']);
...
function SocialMark({ label, monochrome = false }: { label: string; monochrome?: boolean }) {
  ...
  if (key === 'email') return <Mail className="h-[18px] w-[18px]" />;
...
export function PublicHeader({
  ...
  const impactHeader = header.logoUrl?.includes('/impact/') || header.background?.toLowerCase() === '#05070f';
  const brandedHeader = header.logoUrl?.includes('/vielusos/') || impactHeader || header.background?.toLowerCase() === '#0b0b10';
