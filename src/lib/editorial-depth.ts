...
function concreteActions(input: GenerateInput, cause: CauseId, lang: Lang) {
  ...
  if (raw) {
    ...
    if (looksLikeList) {
      return lang === 'en'
        ? `In practical terms, the association can turn these priorities into structured work: ${compact}. These actions should be prepared, explained and followed so visitors understand what happens on the ground and how they can help.`
        : `Concrètement, l’association peut transformer ces priorités en actions structurées : ${compact}. Ces actions doivent être préparées, expliquées et suivies afin que les visiteurs comprennent ce qui se passe sur le terrain et comment ils peuvent aider.`;
    }
    return raw;
  }
  if (cause === 'identity') return lang === 'en'
...
function buildCopy(input: GenerateInput) {
  ...
  return {
    ...
    good,
    homeContext: lang === 'en'
      ? `${created} ${contextFact(cause, lang)}\n\n${mission} The website should help visitors understand not only what the association wants to do, but also why this action matters today and how they can take part without needing technical or administrative knowledge.`
      : `${created} ${contextFact(cause, lang)}\n\n${mission} Le site doit donc permettre aux visiteurs de comprendre non seulement ce que l’association veut faire, mais aussi pourquoi cette action compte aujourd’hui et comment y participer simplement.`,
    homeAction: lang === 'en'
      ? `${name} works with ${who}. On the homepage, the goal is to make the project immediately understandable: who is concerned, what problem is being addressed, why it matters now and what kind of support can be useful.\n\nThis first reading should give volunteers, donors and partners a precise view of the association instead of a generic promise. Visitors can then explore the dedicated pages to discover the detailed actions, impact and ways to get involved.`
      : `${name} agit avec ${who}. Sur l’accueil, l’objectif est de rendre le projet immédiatement compréhensible : qui est concerné, quel problème est traité, pourquoi il compte aujourd’hui et quel type de soutien peut être utile.\n\nCette première lecture donne aux bénévoles, donateurs et partenaires une vision précise de l’association au lieu d’une promesse générique. Les visiteurs peuvent ensuite parcourir les pages dédiées pour découvrir les actions, l’impact et les façons de s’engager.`,
    story: lang === 'en'
      ...
      : `${created} Son histoire ne se résume pas à une date : elle commence lorsque se rencontrent un besoin, une conviction et des personnes prêtes à agir. Autour de ${causeLabel(cause, lang)}, l’association donne une forme publique à des situations souvent mal comprises, minimisées ou laissées aux personnes seules.\n\n${mission} Cette idée fondatrice devient plus solide lorsqu’elle est partagée avec des adhérents, bénévoles et partenaires capables d’apporter une compétence, un réseau ou une expérience de terrain. L’association peut alors grandir sans perdre son but premier : rester utile aux personnes concernées.`,
    methods: lang === 'en'
      ? `${actions}\n\nA useful association is also a method: welcoming without judgement, explaining before asking people to commit, checking that actions answer real needs and keeping information simple. ${good ? `${good} ` : ''}This makes the project more credible for beneficiaries, volunteers and donors.`
      : `${actions}\n\nUne association utile, c’est aussi une méthode : accueillir sans jugement, expliquer avant de demander un engagement, vérifier que les actions répondent à de vrais besoins et garder une information simple. ${good ? `${good} ` : ''}Cette exigence rend le projet plus crédible pour les bénéficiaires, les bénévoles et les donateurs.`,
    impact: lang === 'en'
      ? `The expected impact is not a slogan. It can be observed through concrete signs: people who feel listened to, volunteers who know what to do, partners who understand the project, and actions that can be followed over time.\n\nFor ${who}, progress may mean better information, less isolation, more confidence or a clearer path to support. The association should therefore describe its results carefully, without inventing figures, and use reliable indicators when they exist.`
      : `L’impact attendu n’est pas un slogan. Il peut se lire dans des signes concrets : des personnes qui se sentent écoutées, des bénévoles qui savent comment agir, des partenaires qui comprennent le projet et des actions suivies dans la durée.\n\nPour ${who}, avancer peut vouloir dire être mieux informé, moins isolé, plus confiant ou mieux orienté. L’association doit donc raconter ses résultats avec précision, sans inventer de chiffres, et s’appuyer sur des indicateurs fiables lorsqu’ils existent.`,
    donation: lang === 'en'
      ? `Donations should be presented as a concrete way to strengthen the association’s work, not as a technical payment page. A visitor needs to understand what their contribution can make possible: welcoming people, preparing activities, producing resources, covering essential costs or helping the team respond faster.\n\nThe form below collects the information needed to follow the donation properly and prepare administrative records. If online payment is not connected yet, the association can still receive a pledge and finalise it by bank transfer, cheque, HelloAsso, Leetchi or another enabled method.`
      : `La page de don doit expliquer ce que le soutien rend possible, pas demander à l’association de configurer un outil. Un visiteur doit comprendre à quoi sert sa contribution : accueillir les personnes, préparer les activités, produire des ressources, couvrir des frais essentiels ou permettre à l’équipe de répondre plus vite.\n\nLe formulaire ci-dessous recueille les informations nécessaires pour suivre le don correctement et préparer les justificatifs. Si le paiement en ligne n’est pas encore connecté, l’association peut tout de même recevoir une intention de don puis la finaliser par virement, chèque, HelloAsso, Leetchi ou tout autre moyen activé.`,
    engagement: lang === 'en'
      ...
      : `Soutenir ${name} peut prendre plusieurs formes : bénévolat, adhésion, don, partenariat, relais de communication ou mise à disposition d’une compétence. Pour donner envie d’agir, il faut expliquer ce que chaque contribution rend possible.\n\nUne personne qui découvre l’association doit comprendre rapidement par où commencer : qui contacter, quels besoins sont prioritaires, comment les dons sont utilisés et quelles aides sont utiles. Cette clarté transforme la bonne volonté en participation.`,
    contact: lang === 'en'
      ...
      : `${input.email ? `Email : ${input.email}. ` : ''}${input.city ? `Basée à ${input.city}. ` : ''}Une question, une idée de partenariat ou l’envie d’aider ? Contactez l’association afin que l’équipe puisse vous orienter vers la bonne personne et la bonne forme d’engagement.`,
  };
}
