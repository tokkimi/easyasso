...
export function ShopCatalog({ products, title, intro, search = true, showCategories = true, columns = 4, organizationId = '', canCheckout = false, branded = false }: { products: ShopProduct[]; title?: string; intro?: string; search?: boolean; showCategories?: boolean; columns?: number; organizationId?: string; canCheckout?: boolean; branded?: boolean }) {
  ...
  return <div className={`mx-auto w-full max-w-none px-[5vw] font-light sm:px-[10vw] ${branded ? 'vielusos-shop text-white' : ''}`}>
    ...
    {cartCount > 0 && !cartOpen && <button onClick={() => setCartOpen(true)} className={`fixed bottom-5 right-5 z-50 flex items-center gap-2 rounded-full border px-5 py-3 text-xs font-normal uppercase tracking-[.1em] shadow-2xl ${branded ? 'border-white/20 bg-[#0b0b10] text-white' : 'border-transparent bg-[var(--brand)] text-white'}`}><ShoppingBag className="h-5 w-5" /> Panier · {cartCount} · {euros(cartTotal)}</button>}
...
function ProductModal({ product, branded, isFavorite, onFavorite, onAdd, onClose }: { product: ShopProduct; branded: boolean; isFavorite: boolean; onFavorite: () => void; onAdd: (product: ShopProduct, qty: number, variant?: ShopVariant) => void; onClose: () => void }) {
  ...
  const soldOut = product.stock != null && product.stock <= 0;
  return <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/75 backdrop-blur-sm sm:items-center sm:p-4" onClick={onClose}><div className={`max-h-[92dvh] w-full max-w-4xl overflow-y-auto rounded-t-3xl border sm:rounded-3xl ${branded ? 'border-white/15 bg-[#0b0b10]' : 'border-transparent bg-white'}`} onClick={(event) => event.stopPropagation()}><div className="grid md:grid-cols-[1.2fr_.8fr]"><div className={`relative ${branded ? 'bg-black/40' : 'bg-gray-100'}`}><div className="relative aspect-square">{images[imageIndex] && <img src={images[imageIndex]} alt={`${product.name} — vue ${imageIndex + 1}`} className="h-full w-full object-contain" />}{images.length > 1 && <><button onClick={() => setImageIndex((value) => (value - 1 + images.length) % images.length)} className="absolute left-3 top-1/2 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-full border border-white/20 bg-black/65 text-white"><ChevronLeft /></button><button onClick={() => setImageIndex((value) => (value + 1) % images.length)} className="absolute right-3 top-1/2 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-full border border-white/20 bg-black/65 text-white"><ChevronRight /></button></>}</div>{images.length > 1 && <div className="flex gap-2 overflow-x-auto p-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{images.map((image, index) => <button key={`${image}-${index}`} onClick={() => setImageIndex(index)} className={`h-14 w-14 shrink-0 overflow-hidden rounded-lg border ${imageIndex === index ? 'border-white' : 'border-white/15 opacity-60'}`}><img src={image} alt="" className="h-full w-full object-contain" /></button>)}</div>}</div><div className="relative flex flex-col p-5 md:p-7"><button onClick={onClose} className={`absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-full border ${branded ? 'border-white/15 text-white' : 'border-gray-200 text-gray-700'}`}><X className="h-4 w-4" /></button>{(product.brand || product.category) && <p className={`pr-12 text-[10px] uppercase tracking-[.2em] ${muted}`}>{product.brand}{product.brand && product.category ? ' · ' : ''}{product.category}</p>}<h3 className={`mt-2 pr-12 text-xl font-light uppercase tracking-[.08em] ${strong}`}>{product.name}</h3><p className={`mt-3 text-lg font-normal ${strong}`}>{euros(price)}</p>{product.description && <p className={`mt-4 whitespace-pre-wrap text-sm font-light leading-relaxed ${branded ? 'text-white/70' : 'text-gray-600'}`}>{product.description}</p>}{product.variants?.length ? <label className={`mt-5 text-[11px] uppercase tracking-[.12em] ${muted}`}>Taille / finition<select value={variantId} onChange={(event) => setVariantId(event.target.value)} className={`mt-2 w-full rounded-xl border px-3 py-3 text-sm font-light ${branded ? 'border-white/20 bg-[#15151b] text-white' : 'border-gray-200 bg-white text-gray-900'}`}>{product.variants.map((item) => <option key={item.id} value={item.id}>{item.label || item.id} — {euros(item.priceCents)}</option>)}</select></label> : null}<button type="button" onClick={onFavorite} className={`mt-4 inline-flex w-fit items-center gap-2 text-xs ${muted}`}><Heart className={`h-4 w-4 ${isFavorite ? 'fill-current' : ''}`} />{isFavorite ? 'Dans mes favoris' : 'Ajouter aux favoris'}</button>{!soldOut && <div className="mt-auto flex items-center gap-3 pt-5"><div className="flex items-center rounded-xl border border-current/20"><button onClick={() => setQuantity((value) => Math.max(1, value - 1))} className={`grid h-11 w-10 place-items-center ${muted}`}><Minus className="h-4 w-4" /></button><span className={`w-7 text-center text-sm ${strong}`}>{quantity}</span><button onClick={() => setQuantity((value) => Math.min(99, value + 1))} className={`grid h-11 w-10 place-items-center ${muted}`}><Plus className="h-4 w-4" /></button></div><button onClick={() => onAdd(product, quantity, variant)} className={`flex flex-1 items-center justify-center gap-2 rounded-xl py-3 text-xs font-normal uppercase tracking-[.08em] ${branded ? 'bg-white text-black' : 'bg-[var(--brand)] text-white'}`}><ShoppingBag className="h-4 w-4" />Ajouter au panier</button></div>}</div></div></div></div>;
}
