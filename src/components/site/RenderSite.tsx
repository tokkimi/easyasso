...
type SiteWithPages = NonNullable<
  Awaited<ReturnType<typeof loadSiteBySubdomain>>
>;
...
export async function loadSiteBySubdomain(subdomain: string) {
  return prisma.site.findUnique({
    ...
    include: {
      organization: {
        select: { planStatus: true, trialEndsAt: true, profile: true },
      },
      pages: {
        orderBy: { order: "asc" },
...
export async function loadSiteByDomain(domain: string) {
  ...
  return prisma.site.findFirst({
    where: {
      customDomain: { in: [domain, apex, `www.${apex}`] },
      domainVerified: true,
    },
    include: {
      organization: {
...
export function siteMetadata(
  site: { name: string; header: unknown; footer: unknown } | null,
  subdomain?: string,
): Metadata {
  ...
  const vielusos = isVielusosSite({ subdomain });
  const image = vielusos
    ? vielusosLogoUrl(site)
    : header.logoUrl || footer.logoUrl;
  return {
    title: { absolute: site.name },
    applicationName: vielusos ? "VIELUSOS" : undefined,
    description,
    manifest: vielusos ? "/api/vielusos/manifest" : undefined,
    ...
    openGraph: {
      title: site.name,
      description,
      siteName: vielusos ? "VIELUSOS" : undefined,
      type: "website",
...
export async function RenderSite({
  ...
  const profile = ((site.organization as any)?.profile || {}) as Record<
    string,
    any
  >;
  const vielusos = isVielusosSite(site);
  const brandLogo = vielusosLogoUrl(site);
  const publicHeader = vielusos
    ...
    : header;
  const headerSocials = Object.entries(
  ...
  const columnsWithHeaderSocials = [
    ...footerColumns.filter(
      (column) =>
        !column.links?.length ||
        !column.links.every((link) =>
          socialLabels.has(String(link.label).toLowerCase()),
        ),
    ),
      ...
      ? [
          {
            ...
            links: headerSocials.map(([label, href]) => ({
              label:
                label === "x"
                  ? "X"
                  : socialDisplayLabels[label.toLowerCase()] ||
  ...
  const bubble =
    (footer as any).showContactBubble === false ? null : (
      <ContactBubble
        name={site.name}
      ...
      >
        ...
        <style
          dangerouslySetInnerHTML={{
            __html: `${brandCss(theme.primary)}${vielusos ? VIELUSOS_SITE_CSS : ""}`,
          }}
        />
  ...
  if (slug === "cgv" || slug === "mentions-legales") {
    ...
    return (
      <div
        className={`min-h-screen ${vielusos ? "vielusos-site" : ""}`}
        style={publicSiteStyle(theme, vielusos)}
      >
        {fontHref && <link rel="stylesheet" href={fontHref} />}
        <style
        ...
        <main className="mx-auto max-w-3xl px-4 py-12">
          <h1 className="text-3xl font-extrabold">
            {isCgv ? "Conditions générales" : "Mentions légales"}
          </h1>
          <p className="mt-4 whitespace-pre-wrap leading-relaxed text-gray-600">
            {isCgv ? publicFooter.cgvContent : publicFooter.mentionsContent}
          </p>
        </main>
        <PublicFooter
          footer={publicFooter}
  ...
  const products =
    hasShopBlock && shopEnabled
      ? await loadShopProducts(site.organizationId)
      : [];
    ...
    >
      ...
      {vielusos && page.isHome && (
        <VielusosHero
          title={site.name}
          logoUrl={brandLogo}
          config={(header as any).vielusosHero}
        />
      )}
      {vielusos &&
        (page.slug === "bio" ||
          ...
          <VielusosBio
            ...
            config={(header as any).vielusosBio}
          />
        )}
      <main className="flex-1 py-8">
        ...
        (page.slug === "bio" ||
          page.slug === "about" ||
          page.slug === "a-propos") ? null : page.blocks.length === 0 ? (
...
function ClientAccessPage({
  ...
  return (
    <main
      className={`flex-1 px-4 py-12 ${branded ? "bg-transparent" : "bg-gray-50"}`}
    >
      <section
        className={`mx-auto max-w-2xl rounded-3xl p-5 text-center shadow-sm sm:p-7 md:rounded-[2rem] md:p-10 ${branded ? "bg-[#0b0b10]/55 text-[#f7f7fb] ring-1 ring-white/15 backdrop-blur-xl" : "bg-white ring-1 ring-gray-200"}`}
      >
        <p
          className={`text-sm font-bold uppercase tracking-[0.2em] ${branded ? "text-[#d33f5c]" : "text-[var(--brand)]"}`}
        >
          {branded
            ? "Vielusos · espace client"
            : en
              ? "Customer area"
              : "Espace client"}
        </p>
        <h1
          className={`mt-3 text-2xl font-black sm:text-3xl md:text-4xl ${branded ? "text-white" : "text-gray-900"}`}
        >
          {en
        ...
        <p
          className={`mx-auto mt-4 max-w-xl ${branded ? "text-white/65" : "text-gray-600"}`}
        >
          {en
    ...
    <main className="grid min-h-screen place-items-center bg-gray-50 px-4 py-12 text-center">
      <section className="w-full max-w-lg rounded-[2rem] bg-white p-8 shadow-sm ring-1 ring-gray-200">
        ...
        <p className="mt-4 text-gray-600">
          L’association doit finaliser son activation pour remettre son site en
          ligne.
        </p>
      </section>
    </main>
