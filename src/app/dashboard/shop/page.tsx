...
export default async function ShopPage() {
  const ctx = await requirePermission(PERMISSIONS.SHOP_VIEW);
  const org = ctx.organization!;
  const [products, site, orders, shippingRates] = await Promise.all([
    ...
    prisma.shippingRate.findMany({ where: { organizationId: org.id }, orderBy: [{ order: 'asc' }, { createdAt: 'asc' }] }),
  ]);
  ...
  return (
    <ShopClient
      ...
      shippingRates={JSON.parse(JSON.stringify(shippingRates))}
      canEdit={ctx.permissions.has(PERMISSIONS.SHOP_EDIT)}
    />
  );
}
