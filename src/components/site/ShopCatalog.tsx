...
export function ShopCatalog({ products, title, intro, search = true, showCategories = true, columns = 4, organizationId = '', canCheckout = false, branded = false }: { products: ShopProduct[]; title?: string; intro?: string; search?: boolean; showCategories?: boolean; columns?: number; organizationId?: string; canCheckout?: boolean; branded?: boolean }) {
  ...
  return <div className={`mx-auto w-full max-w-none px-[5vw] font-light sm:px-[10vw] ${branded ? 'vielusos-shop text-white' : ''}`}>
    ...
    <div className={`mt-6 flex items-center justify-between border-y py-3 ${branded ? 'border-white/10' : 'border-gray-200'}`}>
      ...
      {branded ? <a href="https://www.contrado.fr/stores/vielusos" className="inline-flex items-center gap-2 rounded-full border border-white/25 px-4 py-2 text-xs font-normal uppercase tracking-[.12em] text-white hover:bg-white hover:text-black"><LockKeyhole className="h-4 w-4" /> Paiement Contrado</a> : <button type="button" onClick={() => setCartOpen(true)} className="inline-flex items-center gap-2 rounded-full border border-gray-300 px-4 py-2 text-xs font-normal uppercase tracking-[.12em] text-gray-800"><ShoppingBag className="h-4 w-4" /> Panier ({cartCount})</button>}
    </div>
    ...
    {visibleProducts.length === 0 ? <p className={`py-16 text-center ${muted}`}>Aucun produit ne correspond à votre recherche.</p> : <div className={`mt-6 grid gap-6 md:gap-8 ${gridCols}`}>
      {visibleProducts.map((product) => {
        ...
        return <article key={product.id} className="group min-w-0">
          ...
          <div className="mt-3 flex gap-2">{branded && product.checkoutUrl ? <a href={product.checkoutUrl} className="flex flex-1 items-center justify-center gap-2 rounded-full bg-white px-3 py-2.5 text-[11px] font-normal uppercase tracking-[.1em] text-black hover:bg-white/85"><LockKeyhole className="h-4 w-4" /> Acheter avec Contrado</a> : <button type="button" onClick={() => product.variants?.length ? setOpenProduct(product) : addToCart(product)} disabled={soldOut} className={`flex flex-1 items-center justify-center gap-2 rounded-full px-3 py-2.5 text-[11px] font-normal uppercase tracking-[.1em] disabled:opacity-40 ${primary}`}><ShoppingBag className="h-4 w-4" />{product.variants?.length ? 'Choisir les options' : 'Ajouter au panier'}</button>}<button type="button" onClick={() => toggleFavorite(product.id)} className={`grid h-10 w-10 place-items-center rounded-full border ${branded ? 'border-white/20 text-white' : 'border-gray-300 text-gray-600'}`} aria-label="Favori"><Heart className={`h-4 w-4 ${favorites.includes(product.id) ? 'fill-current' : ''}`} /></button></div>
        </article>;
      })}
    </div>}
    ...
    {!branded && cartCount > 0 && !cartOpen && <button onClick={() => setCartOpen(true)} className="fixed bottom-5 right-5 z-50 flex items-center gap-2 rounded-full bg-[var(--brand)] px-5 py-3 text-xs font-normal uppercase tracking-[.1em] text-white shadow-2xl"><ShoppingBag className="h-5 w-5" /> Panier · {cartCount} · {euros(cartTotal)}</button>}
    {!branded && cartOpen && <CartDrawer cart={cart} total={cartTotal} guest={guest} setGuest={setGuest} setQuantity={setQuantity} onClose={() => setCartOpen(false)} onCheckout={checkout} paying={paying} canCheckout={canCheckout} branded={branded} fieldClass={field} primaryClass={primary} />}
  </div>;
}
    ...
    <div className={`max-h-[92dvh] w-full max-w-4xl overflow-y-auto rounded-t-3xl border sm:rounded-3xl ${branded ? 'border-white/15 bg-[#0b0b10]' : 'border-transparent bg-white'}`} onClick={(event) => event.stopPropagation()}>
      <div className="grid md:grid-cols-[1.2fr_.8fr]">
        ...
        <div className="relative flex flex-col p-5 md:p-7">
          ...
          {!branded && product.variants?.length ? <label className={`mt-5 text-[11px] uppercase tracking-[.12em] ${muted}`}>Taille / finition<select value={variantId} onChange={(event) => setVariantId(event.target.value)} className="mt-2 w-full rounded-xl border border-gray-200 bg-white px-3 py-3 text-sm font-light text-gray-900">{product.variants.map((item) => <option key={item.id} value={item.id}>{item.label || item.id} — {euros(item.priceCents)}</option>)}</select></label> : null}
          ...
          {!soldOut && branded && product.checkoutUrl && <div className="mt-auto pt-5">
            <p className="mb-3 text-xs font-light leading-relaxed text-white/55">Choisissez vos options, renseignez la livraison et réglez directement auprès de Contrado.</p>
            <a href={product.checkoutUrl} className="flex w-full items-center justify-center gap-2 rounded-xl bg-white py-3 text-xs font-normal uppercase tracking-[.08em] text-black hover:bg-white/85"><LockKeyhole className="h-4 w-4" />Acheter avec Contrado</a>
          </div>}
          {!soldOut && !branded && <div className="mt-auto flex items-center gap-3 pt-5">
            ...
            <button onClick={() => onAdd(product, quantity, variant)} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[var(--brand)] py-3 text-xs font-normal uppercase tracking-[.08em] text-white"><ShoppingBag className="h-4 w-4" />Ajouter au panier</button>
          </div>}
        </div>
      </div>
    </div>
