      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[.75fr_1.25fr] lg:gap-12">
          <section>
            ...
            <h1 className="mt-4 text-4xl font-semibold uppercase leading-none md:text-5xl" style={{ ...headingFont, textShadow: '0 0 22px rgba(47,107,255,.42)' }}>{en ? copy.titleEn || 'Send a clear brief' : copy.title || 'Envoyer un brief clair'}</h1>
            <p className="mt-6 max-w-md text-sm font-light leading-7 text-white/58 md:text-base">{en ? copy.descriptionEn || 'Booking, media, partnerships or a direct professional enquiry concerning IMPACT.' : copy.description || 'Booking, média, partenariat ou demande professionnelle directe concernant IMPACT.'}</p>
          ...
          <section className="rounded-[2rem] border border-[#4cc9ff]/20 bg-[#070b1a]/40 p-5 shadow-[0_0_60px_-20px_rgba(47,107,255,.7)] backdrop-blur-xl sm:p-8">
            {state === 'sent' ? <div className="grid min-h-[32rem] place-items-center text-center"><div><CheckCircle2 className="mx-auto h-12 w-12 text-[#4cc9ff]" /><h2 className="mt-5 text-3xl text-white" style={headingFont}>{en ? 'Request sent' : 'Demande envoyée'}</h2><p className="mt-3 text-white/50">{en ? 'Thank you. The booking request has been received.' : 'Merci. La demande de booking a bien été reçue.'}</p></div></div> : <form onSubmit={submit} className="space-y-4">
              <div className="flex items-center gap-3 border-b border-[#4cc9ff]/15 pb-5"><CalendarDays className="h-5 w-5 text-[#4cc9ff]" /><div><p className="text-xs uppercase text-white/40">01 · 02 · 03</p><h2 className="mt-1 text-xl font-light uppercase" style={headingFont}>{en ? copy.formTitleEn || 'Contact · Project' : copy.formTitle || 'Contact · Projet'}</h2></div></div>
