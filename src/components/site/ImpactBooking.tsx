...
export function ImpactBooking({ organizationId, copy = {} }: { organizationId: string; copy?: BookingCopy }) {
  ...
  const field = 'impact-booking-field w-full rounded-xl border px-4 py-3.5 text-sm outline-none transition focus:border-[#4cc9ff]/70 focus:shadow-[0_0_18px_rgba(47,107,255,.35)]';
  const headingFont = { fontFamily: '"Space Grotesk", Arial, sans-serif' as const };
  return (
    <main className="impact-booking-page flex-1 px-5 py-10 md:px-10 md:py-16">
      <div className="mx-auto max-w-6xl">
        <div className="flex items-center justify-between gap-4"><a href="/" className="impact-booking-back inline-flex items-center gap-2 text-[10px] font-medium uppercase tracking-[0.28em] transition hover:text-[#2f6bff]"><ArrowLeft className="h-4 w-4" />{en ? 'Back' : 'Retour'}</a></div>
        <div className="mt-8 grid gap-8 lg:grid-cols-[.75fr_1.25fr] lg:gap-12">
          <section>
            <p className="text-[10px] uppercase text-[#9dc7ff]">IMPACT · BOOKING</p>
            <h1 className="impact-booking-title mt-4 text-4xl font-semibold uppercase leading-none md:text-5xl" style={headingFont}>{en ? copy.titleEn || 'Send a clear brief' : copy.title || 'Envoyer un brief clair'}</h1>
            <p className="impact-booking-description mt-6 max-w-md text-sm font-light leading-7 md:text-base">{en ? copy.descriptionEn || 'Booking, media, partnerships or a direct professional enquiry concerning IMPACT.' : copy.description || 'Booking, média, partenariat ou demande professionnelle directe concernant IMPACT.'}</p>
          ...
          <section className="impact-booking-panel rounded-[2rem] border p-5 backdrop-blur-xl sm:p-8">
            {state === 'sent' ? <div className="grid min-h-[32rem] place-items-center text-center"><div><CheckCircle2 className="mx-auto h-12 w-12 text-[#4cc9ff]" /><h2 className="mt-5 text-3xl text-white" style={headingFont}>{en ? 'Request sent' : 'Demande envoyée'}</h2><p className="mt-3 text-white/50">{en ? 'Thank you. The booking request has been received.' : 'Merci. La demande de booking a bien été reçue.'}</p></div></div> : <form onSubmit={submit} className="space-y-4">
              <div className="impact-booking-panel-heading flex items-center gap-3 border-b pb-5"><CalendarDays className="h-5 w-5 text-[#2f6bff]" /><div><p className="impact-booking-step text-xs uppercase">01 · 02 · 03</p><h2 className="mt-1 text-xl font-light uppercase" style={headingFont}>{en ? copy.formTitleEn || 'Contact · Project' : copy.formTitle || 'Contact · Projet'}</h2></div></div>
              <input className="hidden" tabIndex={-1} value={form.website} onChange={(e) => set('website', e.target.value)} />
              <label className="block"><span className="impact-booking-label mb-2 block text-[10px] uppercase tracking-[0.22em]">{en ? 'Your request' : 'Votre demande'}</span><select className={field} value={form.requestType} onChange={(e) => set('requestType', e.target.value)}><option>Booking / date</option><option>Média / interview</option><option>Partenariat / marque</option><option>Autre demande professionnelle</option></select></label>
