...
export function ContactForm({ organizationId, content }: { organizationId?: string; content: any }) {
  ...
  return (
    <div className="grid gap-8 rounded-2xl bg-white p-5 text-left shadow-sm ring-1 ring-gray-200 md:grid-cols-[0.8fr_1.2fr] md:p-8">
      <div>
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2"><input className="input" required placeholder={en ? 'Your name' : 'Votre nom'} value={form.name} onChange={(e) => set('name', e.target.value)} /><input className="input" required type="email" placeholder={en ? 'Your email' : 'Votre e-mail'} value={form.email} onChange={(e) => set('email', e.target.value)} /></div>
        <div className="grid gap-3 sm:grid-cols-2"><input className="input" placeholder={en ? 'Phone (optional)' : 'Téléphone (optionnel)'} value={form.phone} onChange={(e) => set('phone', e.target.value)} /><input className="input" placeholder={en ? 'Subject' : 'Objet'} value={form.subject} onChange={(e) => set('subject', e.target.value)} /></div>
